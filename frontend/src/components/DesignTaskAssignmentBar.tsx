...
export function DesignTaskAssignmentBar({
  ...
  const projectsQuery = useQuery({
    ...
    queryFn: () => fetchDesignProjects(contextDepartmentId, { activeOnly: true }),
  ...
  const fixturesQuery = useQuery({
    ...
    queryFn: () => fetchDesignFixtures(projectId, contextDepartmentId, { activeOnly: true }),
  ...
  const canSubmitWorkflowAction = isReleaseAction ? canChangeFixtureStage : true;
  ...
  // ── Assign Workflow Stage mutation (gates stage, then creates task) ────────

  const assignStageMutation = useMutation({
    mutationFn: () => (
      isReleaseAction
        ? releaseFixtureWorkflow({ fixture_id: fixtureId, department_id: contextDepartmentId })
