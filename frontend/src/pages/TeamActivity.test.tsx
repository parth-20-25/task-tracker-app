...
describe("Team Activity", () => {
  beforeEach(() => {
    ...
    fetchTeamActivity.mockResolvedValue([
      { employee_id: "EMP-1", employee_name: "Rahul Patil", current_task: "25-119 · F01\nDrafting", total_active_tasks: 4, status: "Working" },
      { employee_id: "EMP-2", employee_name: "Neha More", current_task: "No assigned tasks", total_active_tasks: 0, status: "Available" },
    ]);
  });

  it("renders only the compact activity columns and requests once on initial render", async () => {
    render(<TeamActivity />, { wrapper });

    expect(await screen.findByText("Rahul Patil")).toBeInTheDocument();
    for (const heading of ["Employee Name", "Current Task", "Total Active Tasks", "Status"]) {
